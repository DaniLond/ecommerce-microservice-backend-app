...
import pytest
from conftest import make_request, set_current_service, generate_unique_id, is_success_status
...
class TestOrderServiceE2E:
    ...
    @pytest.mark.skip(reason="Backend devuelve 500 Internal Server Error en GET /api/carts")
    def test_e2e_complete_cart_lifecycle(self, cleanup_resources):
        ...
        assert is_success_status(get_response.status_code)
        ...
        assert is_success_status(all_carts_response.status_code)
    ...
    def test_e2e_order_from_cart_workflow(self, cleanup_resources):
        ...
        assert is_success_status(cart_response.status_code)
        ...
        assert is_success_status(order_response.status_code)
        ...
        assert is_success_status(get_order_response.status_code)
        ...
        assert is_success_status(update_order_response.status_code)
    ...
    def test_e2e_multiple_orders_management(self, cleanup_resources):
        ...
        orders_data = [
            {
                ...
                "orderFee": 599.99,
                "cart": {"cartId": created_carts[0]["cartId"]},
            },
            {
                "orderDesc": f"Books_Order_{unique_id}",
                "orderFee": 79.99,
                "cart": {"cartId": created_carts[1]["cartId"]},
            },
            {
                "orderDesc": f"Clothing_Order_{unique_id}",
                "orderFee": 199.99,
                "cart": {"cartId": created_carts[2]["cartId"]},
            },
        ]
        ...
        assert is_success_status(all_orders_response.status_code)
        ...
        for created_order in created_orders:
            ...
            assert is_success_status(specific_order_response.status_code)
    ...
    def test_e2e_order_fee_calculations(self, cleanup_resources):
        ...
        assert is_success_status(discount_response.status_code)
        ...
        assert is_success_status(final_response.status_code)
    ...
    def test_e2e_cart_and_order_bulk_operations(self, cleanup_resources):
        ...
        for i in range(1, 2):
            ...
            assert is_success_status(check_response.status_code)
            ...
            assert is_success_status(cart_check_response.status_code)
