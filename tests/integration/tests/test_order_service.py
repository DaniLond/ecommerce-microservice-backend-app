...
class TestOrderServiceComplete:
    ...
    def create_test_cart(self):
        ...
        assert is_success_status(response.status_code), f"Error al crear carrito: {response.text}"
    ...
    @pytest.mark.skip(reason="Backend devuelve 500 Internal Server Error - problema del servicio, no de las pruebas")
    ...
    def create_test_order(self, create_test_cart):
        ...
        assert is_success_status(response.status_code), f"Error al crear orden: {response.text}"
